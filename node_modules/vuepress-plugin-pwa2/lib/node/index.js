...
`)},M=(e={},a=!1)=>n=>{a&&T(e),n.env.isDebug&&m.info(`Options: ${e.toString()}`);const{base:t,shouldPrefetch:o=!0}=n.options;e.appendBase&&D(t,e),o===!0&&m.warn('The plugin will register service worker to handle assets, so we recommend you to set "shouldPrefetch: false" in VuePress config file.');const l=O(n,e);return n.options.head=B(e,t,n.options.head),k(n,{id:"hope"}),{name:"vuepress-plugin-pwa2",define:()=>({PWA_LOCALES:z({app:n,name:"pwa",default:v,config:e.locales}),SW_FORCE_UPDATE:e.update==="force",SW_PATH:e.swPath||"service-worker.js"}),extendsBundlerOptions:(i,s)=>{S({app:s,config:i},["mitt","register-service-worker"]),x(i,s,"/manifest.webmanifest",async()=>JSON.stringify(await l),"Unexpected manifest generate error")},onGenerated:async i=>{await $(i,l),await W(i,e)},clientConfigFile:i=>L(i,e)}};export{v as pwaLocales,M as pwaPlugin};
